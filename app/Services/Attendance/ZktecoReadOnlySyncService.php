...
class ZktecoReadOnlySyncService
{
 ...
 public function sync(AttendanceMachine $device,?int $userId=null):AttendanceSyncBatch{
  $process=new Process([config('services.zkteco.python'),base_path('scripts/zkteco_readonly_sync.py'),'--ip',(string)$device->host(),'--port',(string)$device->port,'--comm-key',(string)$device->comm_key,'--timeout','8']);$process->setTimeout(30);$process->run();$data=json_decode($process->getOutput(),true);
